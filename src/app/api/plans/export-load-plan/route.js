...
export async function POST(request) {
  try {
    ...
    console.log('=== SERVER: RECEIVED EXPORT REQUEST ===')
    console.log('Has plan:', !!assignment?.plan)
    console.log('Has units:', !!assignment?.units)
    console.log('Has unassigned_orders:', !!assignment?.unassigned_orders)
    console.log('Units count:', assignment?.units?.length || 0)
    console.log(
      'Unassigned orders count:',
      assignment?.unassigned_orders?.length || 0
    )
    console.log('========================================')

    // Use units array from the assignment
    ...
    // Clear ALL columns from B onwards (not just the ones we'll use)
    const startCol = 2 // Column B
    const lastCol = ws.columnCount || 50 // Clear up to last column or 50
    const clearRows = (indices = []) => {
      indices.forEach((r) => {
        if (!r) return
    ...
    units.forEach((unit) => {
      ...
      maxRequiredBlocks = Math.max(maxRequiredBlocks, orders.length)
    })
    ...
    // ------ Sort units by capacity percentage (least full to most full) ------
    const sortedUnits = units.slice().sort((a, b) => {
      const aCapacity = Number(
        a.trailer?.capacity_kg || a.vehicle?.capacity_kg || 0
      )
      const bCapacity = Number(
        b.trailer?.capacity_kg || b.vehicle?.capacity_kg || 0
      )
      const aUsed = Number(a.summary?.total_weight || 0)
      const bUsed = Number(b.summary?.total_weight || 0)
      const aPercentage = aCapacity > 0 ? (aUsed / aCapacity) * 100 : 0
      const bPercentage = bCapacity > 0 ? (bUsed / bCapacity) * 100 : 0
      return aPercentage - bPercentage
    })
    ...
    let col = startCol
    for (const unit of sortedUnits) {
      const isRigid = (unit?.vehicle_type || '').toLowerCase() === 'rigid'
      const capacity_kg = isRigid
        ? Number(unit.vehicle?.capacity_kg || 0)
        : Number(unit.trailer?.capacity_kg || 0)
      const used_kg = Number(unit.summary?.total_weight || 0)
      const diff_kg = capacity_kg - used_kg
      const util = capacity_kg ? used_kg / capacity_kg : 0

      const vehiclePlate = unit?.vehicle?.plate || ''
      const vehicleFleet = unit?.vehicle?.fleet_number || ''
      const trailerPlate = unit?.trailer?.plate || ''
      const driverFullName = unit?.driver?.name || ''
      const driver = driverFullName.split(' ')[0]
      const unitType = unit?.vehicle_type || ''

      // Top block writes (preserve template formatting)
      if (rowIdx.payloadTons) {
        ws.getCell(rowIdx.payloadTons, col).value = capacity_kg / 1000
      }
      if (rowIdx.tonsLoaded) {
        ws.getCell(rowIdx.tonsLoaded, col).value = used_kg / 1000
      }
      if (rowIdx.diffKg) {
        ws.getCell(rowIdx.diffKg, col).value = diff_kg
      }
      if (rowIdx.vehicleMass) {
        ws.getCell(rowIdx.vehicleMass, col).value = capacity_kg
      }
      if (rowIdx.utilPct) {
        ws.getCell(rowIdx.utilPct, col).value = util
      }
      if (rowIdx.vehicleType)
        ws.getCell(rowIdx.vehicleType, col).value = unitType.toUpperCase()
      if (rowIdx.vehicleReg)
        ws.getCell(rowIdx.vehicleReg, col).value = vehiclePlate
      if (rowIdx.fleetNumber)
        ws.getCell(rowIdx.fleetNumber, col).value = vehicleFleet
      if (rowIdx.length)
        ws.getCell(rowIdx.length, col).value = unit?.vehicle?.model || ''
      if (rowIdx.backboard) ws.getCell(rowIdx.backboard, col).value = ''
      if (rowIdx.trailerReg)
        ws.getCell(rowIdx.trailerReg, col).value = trailerPlate
      if (rowIdx.driverName) ws.getCell(rowIdx.driverName, col).value = driver
      ...
      if (firstCustomerRow && maxBlocks) {
        ...
        orders.forEach((order) => {
          ...
          const area = order?.suburb_name || ''
          const so = order?.sales_order_number || ''
          const descs = Array.isArray(order.lines)
            ? order.lines.map((line) => line?.description || '').filter(Boolean)
            : []
          const product = descs.join('; ').slice(0, 200)
          const comments = order?.comments || ''
          const salesPerson = order?.sales_person_name || ''
          const massKg = Number(order?.total_weight || 0)
    ...
    // ------ Populate "Not Loaded" sheet with unassigned orders ------
    const notLoadedSheet = wb.worksheets.find((s) => s.name === 'Not Loaded')
    const unassignedOrders = Array.isArray(assignment?.unassigned_orders)
      ? assignment.unassigned_orders
      : []

    console.log('=== NOT LOADED SHEET DEBUG ===')
    console.log('Sheet found:', !!notLoadedSheet)
    console.log('Unassigned orders count:', unassignedOrders.length)
    console.log('==============================')

    if (notLoadedSheet) {
      stripSharedFormulas(notLoadedSheet)
    }

    if (notLoadedSheet && unassignedOrders.length > 0) {
      let notLoadedRow = 2
      unassignedOrders.forEach((order) => {
        notLoadedSheet.getCell(notLoadedRow, 1).value =
          order.sales_person_name || ''
        notLoadedSheet.getCell(notLoadedRow, 2).value =
          order.sales_order_number || ''
        notLoadedSheet.getCell(notLoadedRow, 3).value =
          order.customer_name || ''
        notLoadedSheet.getCell(notLoadedRow, 4).value =
          order.delivery_date || ''
        notLoadedSheet.getCell(notLoadedRow, 5).value = order.address || ''
        notLoadedSheet.getCell(notLoadedRow, 6).value = ''
        notLoadedSheet.getCell(notLoadedRow, 7).value = ''
        notLoadedSheet.getCell(notLoadedRow, 8).value = ''
        notLoadedSheet.getCell(notLoadedRow, 9).value =
          order.total_quantity || ''
        notLoadedSheet.getCell(notLoadedRow, 10).value =
          order.total_weight || ''
        notLoadedSheet.getCell(notLoadedRow, 11).value = ''
        notLoadedSheet.getCell(notLoadedRow, 12).value = ''
        notLoadedSheet.getCell(notLoadedRow, 13).value = ''
        notLoadedSheet.getCell(notLoadedRow, 14).value = ''
        notLoadedSheet.getCell(notLoadedRow, 15).value = ''
        notLoadedSheet.getCell(notLoadedRow, 16).value = ''
        notLoadedSheet.getCell(notLoadedRow, 17).value = order.status || ''
        notLoadedSheet.getCell(notLoadedRow, 18).value = ''
        notLoadedSheet.getCell(notLoadedRow, 19).value = order.suburb_name || ''
        notLoadedSheet.getCell(notLoadedRow, 20).value = order.route_name || ''
        notLoadedSheet.getCell(notLoadedRow, 21).value = ''
        notLoadedSheet.getCell(notLoadedRow, 22).value = dateTimeStr
        notLoadedRow++
      })
    }

    // ------ Populate "Loaded" sheet with assigned orders ------
    const loadedSheet = wb.worksheets.find((s) => s.name === 'Loaded')

    if (loadedSheet) {
      stripSharedFormulas(loadedSheet)
    }

    if (loadedSheet && units.length > 0) {
      let loadedRow = 2
      units.forEach((unit) => {
        const orders = Array.isArray(unit?.orders) ? unit.orders : []
        orders.forEach((order) => {
          loadedSheet.getCell(loadedRow, 1).value =
            order.sales_person_name || ''
          loadedSheet.getCell(loadedRow, 2).value =
            order.sales_order_number || ''
          loadedSheet.getCell(loadedRow, 3).value = order.customer_name || ''
          loadedSheet.getCell(loadedRow, 4).value = order.delivery_date || ''
          loadedSheet.getCell(loadedRow, 5).value = order.address || ''
          loadedSheet.getCell(loadedRow, 6).value = ''
          loadedSheet.getCell(loadedRow, 7).value = ''
          loadedSheet.getCell(loadedRow, 8).value = ''
          loadedSheet.getCell(loadedRow, 9).value = order.total_quantity || ''
          loadedSheet.getCell(loadedRow, 10).value = order.total_weight || ''
          loadedSheet.getCell(loadedRow, 11).value = ''
          loadedSheet.getCell(loadedRow, 12).value = ''
          loadedSheet.getCell(loadedRow, 13).value = ''
          loadedSheet.getCell(loadedRow, 14).value = ''
          loadedSheet.getCell(loadedRow, 15).value = ''
          loadedSheet.getCell(loadedRow, 16).value = ''
          loadedSheet.getCell(loadedRow, 17).value = order.status || ''
          loadedSheet.getCell(loadedRow, 18).value = ''
          loadedSheet.getCell(loadedRow, 19).value = order.suburb_name || ''
          loadedSheet.getCell(loadedRow, 20).value = order.route_name || ''
          loadedSheet.getCell(loadedRow, 21).value = ''
          loadedSheet.getCell(loadedRow, 22).value = dateTimeStr
          loadedRow++
        })
      })
    }
    ...
    console.log('=== SERVER: RETURNING EXCEL FILE ===')
    console.log('Buffer size:', buffer.length, 'bytes')
    console.log('Filename:', fileName)
    console.log('Units processed:', sortedUnits.length)
    console.log('=====================================')
